...
- Logo variants: `done` — the provided light/dark SVG wordmarks and lockups are theme-aware in the shared brand component; the standalone SVG mark is used as the favicon.
...
- **Reports & JPK export** (mock screen 10): the web page is currently a placeholder; `GET /companies/:id/reports/vat-register` exists, while JPK_V7M XML generation/export is net-new and compliance-sensitive — needs **backend-architect** for the export pipeline and a **tax-advisor** consult on required fields/format before scoping.
...
- **Logo light-theme variant** (found in Phase 5): resolved with the provided themed SVG wordmarks and lockups. `BrandImage.tsx` now selects the correct asset from the explicit Aurora Solid theme, and the standalone mark is used as the favicon.
