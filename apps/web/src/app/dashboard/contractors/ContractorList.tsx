...
export function ContractorList({
  ...
  const turnoverYear = contractors[0]?.turnoverYear;
  const hasRecords = contractors.length > 0;
  ...
  useEffect(() => {
    ...
    setSummary(null);
    getContractorSummary(companyId, effectiveSelectedId, activeEnvironment, selected?.turnoverYear)
      .catch(() => null)
      .then((result) => {
  ...
  }, [activeEnvironment, companyId, effectiveSelectedId]);

  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_372px] lg:items-start">
      <div className={cn('space-y-4', mobileView === 'detail' && 'hidden lg:block')}>
        {missingNipCount > 0 ? (
          <Banner tone="warning">{t.contractors.dataQuality.missingNip(missingNipCount)}</Banner>
        ) : null}

        <div className={cn('flex flex-wrap items-center gap-3', mobileView === 'detail' && 'hidden lg:flex')}>
          <div className="min-w-0 flex-1 sm:max-w-xs">
            <label htmlFor="contractors-search" className="sr-only">{t.contractors.searchPlaceholder}</label>
            <Input
          ...
          <div className="flex items-center gap-1 rounded-control border border-outline bg-surface-panel p-1">
            {(['all', 'active', 'inactive'] as StatusFilter[]).map((filter) => (
              <button
                ...
                className={cn(
                  'min-h-11 rounded-control px-3 py-1.5 text-xs font-semibold transition',
        ...
        <div className={cn(mobileView === 'detail' && 'hidden lg:block')}>
          {filtered.length === 0 ? (
            <div className="rounded-card border border-outline bg-surface-panel p-6 text-center text-sm text-muted">
              <p>{hasRecords ? t.contractors.emptyFiltered : t.contractors.emptyList}</p>
              {hasRecords ? (
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="mt-3 inline-flex min-h-11 items-center rounded-control bg-secondary-surface px-4 font-semibold text-secondary-ink transition hover:bg-surface-raised focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                >
                  {t.contractors.filterAll}
                </button>
              ) : null}
            ...
            <>
              <div className="hidden overflow-hidden rounded-card border border-outline bg-surface-panel lg:block">
                ...
                <thead>
                  <tr className="bg-surface-muted text-left">
                    ...
                    <HeaderCell className="text-right">{turnoverYear ? t.contractors.columns.turnover(turnoverYear) : t.contractors.columns.turnoverLabel}</HeaderCell>
                ...
                <tbody>
                  {filtered.map((contractor) => (
                    ...
                    >
                      <BodyCell className="p-0 font-medium text-foreground">
                        <button
                          type="button"
                          onClick={() => setSelectedId(contractor.id)}
                          aria-pressed={contractor.id === effectiveSelectedId}
                          className="flex min-h-11 w-full items-center px-[18px] text-left transition focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-primary"
                        >
                          {contractor.name}
                        </button>
                      </BodyCell>
                      ...
                      {canEdit ? (
                        <BodyCell className="text-right lg:hidden">
                          <Link
                            ...
                            className="inline-flex min-h-11 items-center text-xs font-semibold text-primary-strong transition hover:text-primary"
                          >
              ...
              <div className="min-w-0 max-w-full overflow-hidden rounded-card border border-outline bg-surface-panel lg:hidden">
                {filtered.map((contractor) => (
                  <button
                    key={contractor.id}
                    type="button"
                    ...
                    aria-pressed={contractor.id === effectiveSelectedId}
                    className="flex min-h-11 w-full min-w-0 items-center justify-between gap-3 border-t border-outline px-4 py-3 text-left transition first:border-t-0 hover:bg-surface-row-hover focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-primary"
                  >
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-medium text-foreground">{contractor.name}</span>
                      <span className={cn(
                        'mt-1 block truncate font-mono text-xs',
                        contractor.nip ? 'text-muted' : 'text-error-ink',
                      )}>
                        {contractor.nip ? `NIP ${contractor.nip}` : t.contractors.nipMissing}
                      </span>
                    </span>
      ...
      {mobileView === 'detail' && selected ? (
        <div className="rounded-card border border-outline bg-chrome p-5 lg:hidden">
          <ContractorDetail
            contractor={selected}
            canEdit={canEdit}
            summary={summary}
    ...
    <div className="flex flex-col gap-3.5">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <p className="break-words text-[17px] font-semibold leading-tight text-foreground">{contractor.name}</p>
          {contractor.nip ? <p className="font-mono text-xs text-muted">NIP {contractor.nip}</p> : null}
        </div>
        <button
          type="button"
          onClick={onClose}
          className="inline-flex min-h-11 items-center text-[12.5px] text-muted transition hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
        >
        ...
        {summary && summary.recentDocuments.length > 0 ? (
          summary.recentDocuments.map((document) => (
            <div key={document.id} className="flex justify-between text-[12.5px]">
              <Link href={`/dashboard/invoices/${document.id}`} className="inline-flex min-h-11 items-center font-mono text-[12px]">
      ...
      {canEdit ? (
        <div className="mt-auto flex gap-2">
          <Button href={`/dashboard/contractors/${contractor.id}/edit`} variant="secondary" className="flex-1">
            {t.contractors.detail.editButton}
          </Button>
          <Button href="/dashboard/invoices/new" className="flex-1">
            {t.contractors.detail.newInvoiceButton}
          </Button>
        </div>
      ) : null}
    </div>
...
function DetailRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-3 text-[12.5px]">
      ...
      <span className="max-w-[190px] break-words text-right font-medium text-foreground">{children}</span>
    </div>
  );
}
