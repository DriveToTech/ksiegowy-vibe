...
```bash
gzip -dc postgresql-<environment>-<timestamp>.sql.gz | docker compose exec -T postgres psql --single-transaction -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB"
```
