...
- **Two databases, restored independently**: business (`POSTGRES_DB`) and household/personal-mode (`HOUSEHOLD_POSTGRES_DB`). Each backup run dumps both, sharing one timestamp, as two separate artifact sets distinguished by `<database-label>` (`business` or `household`):
...
- Select which database to restore with `DB_RESTORE_DATABASE_LABEL=business|household` (default `business`) — a full recovery is two separate restore runs, one per label.
- Each artifact set is a full logical backup of its own database — restoring one never touches the other.
- Remote publishing layout: `<BACKUP_DESTINATION_ROOT>/postgresql/<environment>/<timestamp>/...` (holds both databases' artifact sets)
